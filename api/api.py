...
face_api = FaceRecognitionAPI(
    model_path=download_model(model_url), face_img_path="saved_faces"
)
...
async def process_frame(file: UploadFile = File(...)):
    ...
    try:
        ...
        return JSONResponse(
            content={"image": img_base64, "message": "Frame processed successfully"}
        )
    except Exception as e:
...
async def get_head_count(file: UploadFile = File(...)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error processing head count: {str(e)}"
        )
...
async def add_student(
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=404, detail=f"ReID {reid_num} not found in database"
            )
...
async def get_unknown_faces():
    ...
    try:
        ...
        # Get all faces from database
        if (
            hasattr(face_api.db_manager, "reid_name_map")
            and face_api.db_manager.reid_name_map
        ):
        ...
        return JSONResponse(
            content={"unknown_faces": unknown_faces, "count": len(unknown_faces)}
        )
...
async def get_face_image(reid_num: int):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error retrieving face image: {str(e)}"
        )
